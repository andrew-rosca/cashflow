...
describe('Projection Engine Tests', () => {
  ...
  describe('F016: Projection engine materializes recurring transactions', () => {
    it('should materialize weekly recurring transactions', async () => {
      ...
      await adapter.createTransaction(TEST_USER_ID, {
        ...
        amount: 100,
        date: new Date('2025-12-01'), // Monday Dec 1
      ...
      const startDate = new Date('2025-12-01')
      const endDate = new Date('2025-12-23') // Extended to ensure Dec 22 is included
      ...
      const balance1 = projections.find(p => p.date.getDate() === 1)?.balance
      const balance8 = projections.find(p => p.date.getDate() === 8)?.balance
      const balance15 = projections.find(p => p.date.getDate() === 15)?.balance
      const balance22 = projections.find(p => p.date.getDate() === 22)?.balance
    ...
    it('should materialize monthly recurring transactions', async () => {
      ...
      const dec16 = projections.find(p => p.date.getMonth() === 11 && p.date.getDate() === 16)
    ...
    it('should materialize bi-weekly recurring transactions', async () => {
      ...
      const jan1 = projections.find(p => p.date.getMonth() === 0 && p.date.getDate() === 1)?.balance
      const jan15 = projections.find(p => p.date.getMonth() === 0 && p.date.getDate() === 15)?.balance
      const jan29 = projections.find(p => p.date.getMonth() === 0 && p.date.getDate() === 29)?.balance
      const feb12 = projections.find(p => p.date.getMonth() === 1 && p.date.getDate() === 12)?.balance
  ...
  describe('F017: Projection engine handles recurring with end date', () => {
    it('should stop recurring after end date', async () => {
      ...
      const endDate = new Date('2026-04-01') // Extended to ensure Mar 31 is included
      ...
      const mar16 = projections.find(p => p.date.getDate() === 16)?.balance
      const mar23 = projections.find(p => p.date.getDate() === 23)?.balance
      const mar31 = projections.find(p => p.date.getDate() === 31)?.balance
    ...
    it('should stop recurring after occurrences limit', async () => {
      ...
      const endDate = new Date('2026-05-01') // Extended to ensure Apr 30 is included
      ...
      const apr8 = projections.find(p => p.date.getDate() === 8)?.balance
      const apr15 = projections.find(p => p.date.getDate() === 15)?.balance
      const apr22 = projections.find(p => p.date.getDate() === 22)?.balance
      const apr30 = projections.find(p => p.date.getDate() === 30)?.balance
  ...
  describe('F018: Projection engine identifies dates where balance <= 0', () => {
    it('should identify when balance drops to zero', async () => {
      ...
      const expenseDate = new Date('2026-05-05T12:00:00Z')
      await adapter.createTransaction(TEST_USER_ID, {
      ...
      const may4 = projections.find(p => p.date.getDate() === 4)?.balance
      const may5 = projections.find(p => p.date.getDate() === 5)?.balance
    ...
    it('should identify multiple danger periods', async () => {
      ...
      const endDate = new Date('2026-06-20T23:59:59Z')
      ...
      const jun4 = projections.find(p => p.date.getDate() === 4)?.balance
      const jun5 = projections.find(p => p.date.getDate() === 5)?.balance
      const jun10 = projections.find(p => p.date.getDate() === 10)?.balance
      const jun15 = projections.find(p => p.date.getDate() === 15)?.balance
  ...
  describe('Settlement lag integration', () => {
    it('should handle settlement lag in projections', async () => {
      ...
      const jul4_acc2 = proj2.find(p => p.date.getDate() === 4)?.balance
